# Unity Multi-Platform XRDP Editor Image

## `unityci/multi-platform-xrdp-editor`

Unity Editor image with integrated support for Windows, Android, and iOS builds plus XRDP GUI access.
...
```bash
docker run -it --rm unityci/multi-platform-xrdp-editor:[tag] bash
```

Example with specific version:

```bash
docker run -it --rm unityci/multi-platform-xrdp-editor:ubuntu-2022.3.0f1-android-ios-windows-1.0.0 bash
```
...
```bash
docker run -d -p 3389:3389 --name unity-desktop unityci/multi-platform-xrdp-editor:[tag]
```

Use any RDP client with:
...
- Headless builds are performed in `xvfb-run`
- GUI workflows are available through XRDP and `unity-editor-gui`
